...
<body id="page-top">
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <?php if (isset($status->video_guru)) { ?>
            <div class="row">
              <!-- Area Recorder -->
              <div class="col-xl-12 col-lg-12">
                <div class="card shadow mb-4">
                  ...
                  <div class="card-header py-3">
                    <h3 class="m-0 font-weight-bold text-primary">Koreksi Guru</h3>
                  </div>
                  <!-- Card Body -->
                  <div class="card-body text-center">
                    <video controls>
                      <source src="<?php echo base_url('uploads/koreksi/'.$status->video_guru) ?>" type="video/webm">
                    </video>
                    <div class="row">
                    ...
                    <?php if (isset($status->pesan_guru)) { ?>
                      Pesan guru Anda: <?php echo $status->pesan_guru; ?>
                    <?php } ?>
                  </div>
                </div>
              </div>
          ...
          <div class="row">
            ...
            <div class="col-xl-12 col-lg-12">
              <div class="card shadow mb-4">
                <!-- Card Header - Dropdown -->
                <div class="card-header py-3">
                  <h3 class="m-0 font-weight-bold text-primary">Rekam <?php echo $materi->nama_materi.' '.$materi->arab_materi.', Huruf '.$submateri->nama_submateri.' ('.$submateri->huruf_submateri.')' ?></h3>
                </div>
                <!-- Card Body -->
                <div class="card-body text-center">
          ...
          <div class="row">
            ...
            <div class="col-xl-12 col-lg-12">
              <div class="card shadow mb-4">
                ...
                <!-- Card Body -->
                <div class="card-body">
                  <p class="text-justify" style="font-size:14pt">Jika Anda sudah selesai merekam, Anda dapat memilih salah satu tombol aksi dibawah hasil video Anda. Pilih unduh untuk mengunduh ke perangkat Anda. Jika Anda sudah yakin pengucapan sudah benar, pilih kumpulkan untuk mengumpulkan ke guru Anda. Namun jika Anda belum yakin, Anda dapat memilih bagikan ke forum untuk membagikan ke forum diskusi dan meminta pendapat teman-teman Anda.</p>
  ...
  <!-- End of Page Wrapper -->
    ...
    <div class="modal fade" id="shareModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Bagikan ke Forum</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body">
                  <div class="form-group">
                    <label>Tulis sesuatu tentang video rekaman Anda</label>
                    <textarea id="isi" rows="3" class="form-control"></textarea>
                  </div>
                </div>
                <div class="modal-footer">
                    <button class="btn btn-secondary" type="button" data-dismiss="modal">Kembali</button>
    ...
    <div class="modal fade" id="selfcorrection" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Penilaian hasil rekaman Anda</h5>
                </div>
                <form id="formselfcorrection" method="POST">
                  <input type="hidden" name="id_materi" value="<?php echo $materi->id_materi ?>">
                  <input type="hidden" name="id_submateri" value="<?php echo $submateri->id_submateri ?>">
                  <div class="modal-body text-justify">
                    Nilailah hasil rekaman Anda yang sudah Anda kumpulkan atau bagikan tadi. Apakah hasil rekaman Anda sudah sesuai dengan teori atau belum.<br><br>
                    <div class="row form-group">
                      <label class="col-6">Tempat keluarnya huruf</label>
                      <div class="col-6">
                    ...
                    <div class="row form-group">
                      <label class="col-6">Suara</label>
                      <div class="col-6">
                        <select class="form-control" name="suara">
                          <option value="1">Kurang</option>
                          <option value="2">Cukup</option>
                          <option value="3">Bagus</option>
                          <option value="4">Sangat Bagus</option>
    ...
    <script>
      ...
      function stopRecording() {
        ...
        $.ajax({
         url:"<?php echo site_url('Controller_siswa/analitik/rekam/'.$materi->nama_materi.'/'.$submateri->nama_submateri) ?>",
         method:"POST",
        ...
        const filename = "<?php echo str_replace(' ', '', $this->session->userdata('username').$materi->nama_materi.$submateri->nama_submateri) ?>";
        ...
        for(var i = 0, td; i < rows.length; i++){
            ...
            dnld.download = "record"+(i+1)+".mp4";
            video.onplaying = function(){playvideo()};
            function playvideo(){
              $.ajax({
               url:"<?php echo site_url('Controller_siswa/analitik/playrecord/'.$materi->nama_materi.'/'.$submateri->nama_submateri) ?>",
               method:"POST",
               data:{id:<?php echo $this->session->userdata('id'); ?>},
               dataType:"JSON",
               success:function(data){
        ...
        upld.addEventListener("click", function(event){
          ...
          if (konfirmasi) {
            ...
            fd.append("materi", <?php echo $materi->id_materi;?>);
            fd.append("submateri", <?php echo $submateri->id_submateri;?>);
            xhr.open("POST","<?php echo site_url('Controller_siswa/uploadtugas/'.$submateri->id_submateri) ?>",true);
            xhr.send(fd);
            $('#loadingModal').modal('show');
            xhr.addEventListener('load', function(){
              document.getElementById('loading').innerHTML = '<i class="fa fa-check fa-5x"></i>&nbsp;&nbsp;&nbsp;Video berhasil terunggah';
              setTimeout(function(){
                $('#selfcorrection').modal('show');
                selfcorrection = 'kumpulkan';
              }, 3000);
            });
        ...
        shr.className = "btn btn-primary m-1";
        shr.href = "#";
        shr.innerHTML = "<i class='fa fa-share'></i> Bagikan ke Forum";
        shr.addEventListener("click", function(event){
          event.preventDefault();
          ...
          $('#kirim').click(function(){
            ...
            var fd = new FormData();
            fd.append("video_data", blob, '<?php echo random_string('alpha', 8); ?>');
            fd.append("isi", $('#isi').val());
            fd.append("materi", "<?php echo $materi->nama_materi; ?>");
            fd.append("huruf", "<?php echo $submateri->nama_submateri; ?>");
            xhr.open("POST","<?php echo site_url('Controller_siswa/sharetugas') ?>",true);
            xhr.send(fd);
            $('#loadingModal').modal('show');
            xhr.addEventListener('load', function(){
              document.getElementById('loading').innerHTML = '<i class="fa fa-check fa-5x"></i>&nbsp;&nbsp;&nbsp;Video berhasil terunggah';
      ...
      $('#formselfcorrection').on('submit', function(event){
        event.preventDefault();
        var form_data = $(this).serialize();
        $.ajax({
         url:"<?php echo site_url('Controller_siswa/selfcorrection') ?>",
         method:"POST",
         data:form_data,
         success:function(data){
          if (selfcorrection == 'kumpulkan') {
            window.location.replace("<?php echo site_url('tugas') ?>");
          }else{
            window.location.replace("<?php echo site_url('forum') ?>");
          }
         }
        });
      });
