...
class Model_login extends CI_Model{
	...
	function analitik($data){
		...
		$materi = $data['materi'];
		$huruf = $data['huruf'];
		$query = $this->db->query("SELECT * FROM analitik WHERE id_user = '$id_user' AND kegiatan = '$kegiatan' AND huruf = '$huruf'");
		if (($kegiatan == 'rekam' && $query->num_rows() < 4) || ($kegiatan == 'playrecord' && $query->num_rows() < 5) || ($kegiatan == 'bagikan' && $query->num_rows() < 3) || $kegiatan == 'diskusi') {
