    ...
    <div class="card shadow mb-4" id="posting<?php echo $listposting->id_posting ?>">
      ...
      <div class="card-body">
        <?php if (isset($listposting->video)) { ?>
          ...
          <?php if ($this->session->userdata('id') != $listposting->id_user && $this->session->userdata('level') == 3) { ?>
            <button id="berinilai<?php echo $listposting->id_posting ?>" class=" btn btn-block btn-info mb-2">Beri Nilai</button>
            <div id="penilaian<?php echo $listposting->id_posting ?>">
              <div class="row">
              ...
              <div class="row">
                <div class="col-4 text-center">
                  <select class="form-control" id="tempat<?php echo $listposting->id_posting ?>">
                ...
                <div class="col-4 text-center">
                  <select class="form-control" id="sifat<?php echo $listposting->id_posting ?>">
                    <option value="1">Kurang</option>
                    <option value="2">Cukup</option>
                    <option value="3">Bagus</option>
                    <option value="4">Sangat Bagus</option>
                ...
                <div class="col-4 text-center">
                  <select class="form-control" id="suara<?php echo $listposting->id_posting ?>">
                    <option value="1">Kurang</option>
                    <option value="2">Cukup</option>
                    <option value="3">Bagus</option>
                    <option value="4">Sangat Bagus</option>
              ...
              <div class="row mt-2">
                <div class="col-6">
                  <button id="submitnilai<?php echo $listposting->id_posting ?>" class="btn btn-block btn-success">Nilai</button>
                </div>
                <div class="col-6">
                  <button id="tutupnilai<?php echo $listposting->id_posting ?>" class="btn btn-block btn-secondary">Tutup</button>
                </div>
              </div>
            </div>
          <?php } ?>
              ...
              <tr>
                <div class="text-danger mb-2" style="display: none;" id="validasi<?php echo $id ?>">Komentar tidak boleh kosong</div>
                  <td style="padding-right: 10px; width: 100%">
                    ...
                    <?php if (isset($listposting->video)) { ?>
                      <input type="hidden" name="ada" value="adavideo">
                    ...
                    <?php }else{ ?>
                      <input type="hidden" name="ada" value="gakada">
                    <?php } ?>
...
<script>
    ...
    <?php foreach ($posting as $listposting) { ?>
      $(document).ready(function(){
        ...
        $('#penilaian<?php echo $listposting->id_posting ?>').hide();
        $.ajax({
          url:"<?php echo site_url('Controller_home/statusnilaishare') ?>",
          method:"POST",
          data:{id_posting:<?php echo $listposting->id_posting?>, id_penilai:<?php echo $this->session->userdata('id')?>},
          cache: false,
          dataType:"JSON",
          success:function(data){
            if (data.ada == 'ada') {
              $('#berinilai<?php echo $listposting->id_posting ?>').html('Anda sudah menilai');
              document.getElementById('berinilai<?php echo $listposting->id_posting ?>').disabled = true;
            }
          }
        });
      ...
      $('#berinilai<?php echo $listposting->id_posting ?>').on('click', function(event){
        event.preventDefault();
        $('#penilaian<?php echo $listposting->id_posting ?>').show(100);
        $('#berinilai<?php echo $listposting->id_posting ?>').hide(100);
      });

      $('#submitnilai<?php echo $listposting->id_posting ?>').on('click', function(event) {
        event.preventDefault();
        var konfirmasi = confirm('Nilai rekaman ini?');
        var materi = "<?php echo $listposting->video_materi; ?>";
        var submateri = "<?php echo $listposting->video_submateri; ?>";
        var tempat = $('#tempat<?php echo $listposting->id_posting ?>').val();
        var sifat = $('#sifat<?php echo $listposting->id_posting ?>').val();
        var suara = $('#suara<?php echo $listposting->id_posting ?>').val();
        if (konfirmasi) {
          $.ajax({
            url: "<?php echo site_url('Controller_home/nilaishare') ?>",
            method: "POST",
            data: {id_posting:<?php echo $listposting->id_posting?>, id_user:<?php echo $listposting->id_user; ?>, id_penilai:<?php echo $this->session->userdata('id')?>, video_materi:materi, video_submateri: submateri, tempat:tempat, sifat:sifat, suara:suara},
            dataType: "JSON",
            success:function(data){
              $('#penilaian<?php echo $listposting->id_posting ?>').hide(100);
              $('#berinilai<?php echo $listposting->id_posting ?>').show(100);
              $('#berinilai<?php echo $listposting->id_posting ?>').html('Anda sudah menilai');
              document.getElementById('berinilai<?php echo $listposting->id_posting ?>').disabled = true;
            }
          });
      ...
      $('#tutupnilai<?php echo $listposting->id_posting ?>').on('click', function(event) {
        event.preventDefault();
        $('#berinilai<?php echo $listposting->id_posting ?>').show(100);
        $('#penilaian<?php echo $listposting->id_posting ?>').hide(100);
      });
