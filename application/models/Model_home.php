...
class Model_home extends CI_Model{
	...
	function listposting($limit, $start){
		$this->db->distinct();
		$this->db->select('home.id_posting, home.status_posting, home.username_user, home.nama_user, home.isi_posting, home.video, home.video_materi, home.video_submateri, home.waktu_posting, home.edit_status, user.id_user, user.foto_user');
		$this->db->from('home');
		$this->db->join('user', 'home.username_user = user.username_user', 'inner');
		$this->db->where('status_posting', 0);
		$this->db->order_by('id_posting', 'desc');
	...
	function statusnilaishare($id_penilai, $id_posting){
		$query = $this->db->query("SELECT * FROM penilaianshare WHERE id_posting = '$id_posting' AND id_penilai = '$id_penilai'");
		return $query->num_rows();
	}

	function nilaishare($data){
		$this->db->insert('penilaianshare', $data);
	}
