...
class Model_siswa extends CI_Model{
	...
	function selfcorrection($data){
		$id = $data['id_user'];
		$id_materi = $data['id_materi'];
		$id_submateri = $data['id_submateri'];
		$query = $this->db->query("SELECT * FROM penilaiancreateplay WHERE id_user = '$id' AND id_materi = '$id_materi' AND id_submateri = '$id_submateri'");
		if ($query->num_rows() > 0) {
			$this->db->set($data);
			$this->db->where('id_user', $id);
			$this->db->where('id_materi', $id_materi);
			$this->db->where('id_submateri', $id_submateri);
			$this->db->update('penilaiancreateplay');
		}else{
			$this->db->insert('penilaiancreateplay', $data);
		}
	}
